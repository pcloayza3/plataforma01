...
| **RF-05** | **Motor de Custodia (Escrow) y Pago por Adelantado** | El estudiante debe pagar por adelantado en moneda local mediante dLocal for Platforms, con opción de pagar hito por hito o el monto total del proyecto. Para iniciar cualquier hito, los fondos correspondientes deben estar resguardados por adelantado en custodia (*escrow*). La plataforma resguarda el dinero durante el desarrollo del hito y aplica comisiones dinámicas:<br>- Monto < 300 BOB: **15%** de comisión.<br>- Monto ≥ 300 BOB: **10%** de comisión. | Fondos registrados en estado `EN_CUSTODIA`. El sistema bloquea el agendamiento y la entrega del hito si no cuenta con pago previo confirmado. |
| **RF-06** | **Gestión de Hitos y Liquidación al Consultor** | El pago se liquida al consultor de forma progresiva al completar y aprobar cada hito. Al presionar el estudiante `Aprobar Hito`, el motor de custodia ejecuta el *pay-out* neto del hito correspondiente a la cuenta local del consultor. Para iniciar el siguiente hito, se valida que esté cubierto por el pago total inicial o se exige el pago anticipado del nuevo hito. | Al confirmar la aprobación del hito, se transfiere automáticamente el monto neto al consultor vía dLocal for Platforms y se actualiza el avance porcentual acumulado. |
